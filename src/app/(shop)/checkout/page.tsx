...
import {
  ArrowLeft, CreditCard, MapPin, Truck, Store, Check, Send,
} from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const [form, setForm] = useState({
    ...
    customer_note: '',
    shipping_line1: '', shipping_line2: '', shipping_city: '', shipping_state: 'GA', shipping_zip: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      localStorage.setItem('sh-customer', JSON.stringify({
        ...
        customer_phone: form.customer_phone, shipping_line1: form.shipping_line1,
        shipping_line2: form.shipping_line2, shipping_city: form.shipping_city,
        shipping_state: form.shipping_state, shipping_zip: form.shipping_zip,
      }));
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          items: items.map((item) => ({
            ...
            quantity: item.quantity, selected_options: item.selectedOptions,
          })),
          subtotal, shipping_fee: shippingFee, discount_amount: discount, total,
        }),
      });
      const data = await res.json();
      if (res.ok) { setOrderComplete(data.order_number); clearCart(); }
      else { toast.error(data.error || '주문 처리 중 오류가 발생했습니다'); }
  ...
  const handlePaymentNotify = async () => {
    ...
    try {
      ...
      if (res.ok) { setNotifySent(true); toast.success('입금 확인 요청이 전송되었습니다!'); }
      else { toast.error('전송 실패. 잠시 후 다시 시도해주세요.'); }
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => updateForm('fulfillment_type', 'pickup')} className={cn('p-4 rounded-xl border-2 text-left transition-all', form.fulfillment_type === 'pickup' ? 'border-brand-500 bg-brand-50' : 'border-slate-200 hover:border-slate-300')}>
                <Store className="w-5 h-5 text-brand-500 mb-2" /><p className="text-sm font-semibold">픽업</p><p className="text-xs text-slate-500 mt-0.5">Atlanta, GA</p>
              </button>
              <button type="button" onClick={() => updateForm('fulfillment_type', 'shipping')} className={cn('p-4 rounded-xl border-2 text-left transition-all', form.fulfillment_type === 'shipping' ? 'border-brand-500 bg-brand-50' : 'border-slate-200 hover:border-slate-300')}>
                <Truck className="w-5 h-5 text-brand-500 mb-2" /><p className="text-sm font-semibold">배송</p><p className="text-xs text-slate-500 mt-0.5">{subtotal >= SHIPPING.FREE_THRESHOLD ? '무료배송' : `$${SHIPPING.FLAT_RATE}`}</p>
              </button>
            </div>
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {(['zelle', 'venmo'] as const).map((method) => (
                <button key={method} type="button" onClick={() => updateForm('payment_method', method)} className={cn('p-4 rounded-xl border-2 text-center transition-all', form.payment_method === method ? 'border-brand-500 bg-brand-50' : 'border-slate-200 hover:border-slate-300')}>
                  <CreditCard className="w-5 h-5 text-brand-500 mx-auto mb-2" /><p className="text-sm font-semibold capitalize">{method}</p><p className="text-xs text-slate-500 mt-0.5">{method === 'zelle' ? 'pay@styleheba.com' : '@styleheba'}</p>
                </button>
              ))}
            </div>
            ...
            <div className="space-y-3 mb-4">
              {items.map((item) => (
                <div key={`${item.product.id}-${JSON.stringify(item.selectedOptions)}`} className="flex gap-3">
                  <div className="w-14 h-14 rounded-lg overflow-hidden bg-slate-100 flex-shrink-0"><img src={getStorageUrl(item.product.thumbnail)} alt="" className="w-full h-full object-cover" /></div>
                  <div className="flex-1 min-w-0"><p className="text-sm font-medium text-slate-800 line-clamp-2">{item.product.name_ko || item.product.name}</p><p className="text-xs text-slate-400">{formatPrice(item.product.price)} × {item.quantity}</p></div>
