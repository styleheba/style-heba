  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        ...
        <div className="card p-3 text-center">
          <p className="text-xs text-slate-400">입금대기</p>
          <p className="text-xl font-bold text-amber-600">{stats.pending}</p>
        </div>
        <div className="card p-3 text-center">
          <p className="text-xs text-slate-400">입금확인</p>
          <p className="text-xl font-bold text-blue-600">{stats.paid}</p>
        </div>
        <div className="card p-3 text-center">
          <p className="text-xs text-slate-400">매출</p>
          <p className="text-xl font-bold text-brand-600">{formatPrice(stats.revenue)}</p>
        </div>
      </div>
      ...
      <div className="card p-3 mb-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select value={fulfillmentFilter} onChange={(e) => setFulfillmentFilter(e.target.value)} className="input-base py-1.5 text-sm w-auto">
            <option value="">전체 수령</option>
            <option value="pickup">픽업</option>
            <option value="shipping">배송</option>
          </select>
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="input-base py-1.5 text-sm w-auto" placeholder="시작일" />
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="input-base py-1.5 text-sm w-auto" placeholder="종료일" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/80">
                <th className="text-left px-3 py-2 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-700 select-none" onClick={() => toggleSort('order_number')}>
                  <span className="flex items-center gap-1">주문번호 <SortIcon field="order_number" /></span>
                </th>
                <th className="text-left px-3 py-2 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-700 select-none" onClick={() => toggleSort('created_at')}>
                  <span className="flex items-center gap-1">날짜 <SortIcon field="created_at" /></span>
                </th>
                <th className="text-left px-3 py-2 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-700 select-none" onClick={() => toggleSort('customer_name')}>
                  <span className="flex items-center gap-1">고객 <SortIcon field="customer_name" /></span>
                </th>
                ...
                <th className="text-center px-3 py-2 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-700 select-none" onClick={() => toggleSort('status')}>
                  <span className="flex items-center justify-center gap-1">상태 <SortIcon field="status" /></span>
                </th>
                <th className="text-right px-3 py-2 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-700 select-none" onClick={() => toggleSort('total')}>
                  <span className="flex items-center justify-end gap-1">합계 <SortIcon field="total" /></span>
                </th>
                ...
                <>
                  <tr key={order.id} className={cn('border-b border-slate-50 hover:bg-slate-25 transition-colors cursor-pointer', expandedId === order.id && 'bg-brand-50/30')} onClick={() => setExpandedId(expandedId === order.id ? null : order.id)}>
                    <td className="px-3 py-2">
                      <span className="text-xs font-mono font-medium text-brand-600">{order.order_number}</span>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <p className="text-xs font-medium text-slate-800 truncate max-w-[100px]">{order.customer_name}</p>
                    </td>
                    <td className="px-3 py-2">
                    ...
                    <td className="px-3 py-2 text-center">
                      <span className="text-[10px] font-medium text-slate-500">{PAYMENT_LABELS[order.payment_method]}</span>
                    </td>
                    <td className="px-3 py-2 text-center" onClick={(e) => e.stopPropagation()}>
                      {editingStatus === order.id ? (
                        <select
                          ...
                          autoFocus
                          className="input-base py-0.5 text-[11px] w-auto"
                        >
                          {STATUS_FLOW.map(s => <option key={s} value={s}>{ORDER_STATUS_LABELS[s]}</option>)}
                        </select>
                      ) : (
                        <button
                          ...
                          className={cn('badge text-[10px] cursor-pointer hover:ring-2 hover:ring-brand-200 transition-all', ORDER_STATUS_COLORS[order.status] || 'bg-slate-100 text-slate-600')}
                        >
                    ...
                    <td className="px-3 py-2 text-center">
                      <Eye className={cn('w-3.5 h-3.5 transition-colors', expandedId === order.id ? 'text-brand-500' : 'text-slate-300')} />
                    </td>
                  </tr>
                      ...
                      <td colSpan={9} className="px-4 py-3">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                              ...
                              <div key={idx} className="flex justify-between">
                                <span className="text-slate-600">
                                  ...
                                  {item.selected_options && Object.keys(item.selected_options).length > 0 && (
                                    <span className="text-slate-400 ml-1">({Object.values(item.selected_options).join('/')})</span>
                                  )}
                                </span>
                          ...
                          <div className="space-y-1.5">
                            ...
                            {order.customer_note && (
                              <div className="p-2 rounded bg-amber-50 text-amber-700 text-[11px]">고객: {order.customer_note}</div>
                            )}
                            <div>
                              ...
                              <textarea
                                ...
                                className="input-base py-1.5 text-xs h-16 resize-none"
